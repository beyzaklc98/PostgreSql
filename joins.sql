...
/*
JOINS: 1)INNER JOIN: Ortak (Comman) datayı verir.
       2)LEFT JOIN: Birinci table'ın tum datasını verir.
	   3)RIGHT JOIN: İkinci table'ın tum datasını verir. Ortak olmayanlara null geliyor.
	   4)FULL JOIN: İki table'ın tum datasını verir.
	   5)SELF JOIN: Tek table uzerinde calısırken iki table varmıs gibi calısır.
*/

--1) INNER JOIN:
--Ortak companyler icin company_name, order_id ve order_date degerlerini cagırın.
Select company_name, order_id, order_date
from my_companies inner join orders
ON  my_company_id = my_company_id;
...
SELECT mc.company_name, o.order_id, o.order_date --name'i gormek icin cagırdık.
