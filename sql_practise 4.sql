...
where isyeri in(select marka_isim, calisan_sayisi from markalar where calisan_sayisi>15000);
-- in kullanacaksanız parantez icine aldıgınız sorgu birden fazla deger dondermeli

select * from calisanlar; -- tablo icin birlesme isyeri sutunu
...
select marka_id,marka_isim, (select count(sehir) from calisanlar where marka_isim=isyeri) as marka_sehir from markalar;
